...
use `wrap_element()` to add non-`ggplot` element to a patch and `par()` to control alignment
```{r}
...
# BCa: accelerated bias-corrected, which seeks to correct for skewness as well as bias in the bootstrap distribution
```
